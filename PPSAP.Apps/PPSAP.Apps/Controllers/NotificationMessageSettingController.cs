...
{
    [Authorize]
    public class NotificationMessageSettingController : Controller
    {
        ...
        public ActionResult GetAllNotificationMessageSetting()
        {
            try
            {
                ...
                string url = PPSAPGlobalConstants.SiteWebAPIUrl + "NotificationMessageSetting/GetAllNotificationMessageSetting";
                ...
                dataresult = Json(report, JsonRequestBehavior.AllowGet);
                return (dataresult);
            }
            catch (Exception e)
            {
        ...
        public ActionResult GetMessageSettingForm(int id)
        {
            ...
            if (id != 0)
            {
                ...
                string examPostDataJson = JsonConvert.SerializeObject(searchParameters);
                string url = PPSAPGlobalConstants.SiteWebAPIUrl + "NotificationMessageSetting/GetNotificationMessageSetting";
                string result = HttpProxy.HttpPost(url, examPostDataJson, "application/json; charset=utf-8", "POST");
        ...
        public ActionResult DeleteMessageSetting(int id)
        {
            ...
            if (id != 0)
            {
                ...
                string examPostDataJson = JsonConvert.SerializeObject(searchParameters);
                string url = PPSAPGlobalConstants.SiteWebAPIUrl + "NotificationMessageSetting/DeleteMessageSetting";
                string result = HttpProxy.HttpPost(url, examPostDataJson, "application/json; charset=utf-8", "POST");
        ...
        [HttpPost]
        public ActionResult SaveNotificationMessageSetting(int NotificationMessageSettingId, string NotificationMessage, int NoOfTime, int? OnholdScreenTime, int? IsEnable)
        {
            bool isSucess = true;
            var msg = string.Empty;
            if (!string.IsNullOrEmpty(NotificationMessage))
            {
                ...
                string url = PPSAPGlobalConstants.SiteWebAPIUrl + "NotificationMessageSetting/SaveNotificationMessageSetting";
                HttpProxy.HttpPost(url, examPostDataJson, "application/json; charset=utf-8", "POST");
            }

            //return Json(new { msg, isSucess });
            return RedirectToAction("Index", "NotificationMessageSetting");
        }
    }
}