$(document).ready(function () {
    ...
    if (BcscMapperViewModel.ProgramId != '') {
        ...
        $("#SearchByIdTitle").on('click', function () {
            ...
            window.location.href = '/AllQuestions/Index' + '?pageIndex=' + pageIndex + '&noOfRecords=' + noOfRecords;
        });
    ...
    $("#resetQuestions").on('click', function () {
        window.location.href = '/AllQuestions/Index';
    });
    ...
    $('input:radio[name="question"]').click(function () {
        ...
        if ($('input:radio[name="question"]:checked')[0].value == "ID") {
            var changedVal = $("#search").attr("placeholder", "Search the question by Id ").placeholder();
    ...
    function SearchByTitle(pageIndex, RadioButtonState) {
        ...
        if (!($.isNumeric(QuestionText)) || (regex.test(QuestionText) == true)) {
            window.location.href = '/AllQuestions/GetQuestionByTitle' + '?QuestionText=' + QuestionText + '&pageIndex=' + pageIndex + '&noOfRecords=' + noOfRecords + '&radioButtonState=' + radioButtonState;
        }
        else {
    ...
    function SearchByID() {
        ...
        if ($.isNumeric(QuestionId)) {
            window.location.href = '/AllQuestions/GetQuestionById' + '?QuestionId=' + QuestionId;
        }
        else {
   ...
   /* $('#ProgramBox').on('change', function () {*/
        ...
        if (BcscMapperViewModel.ProgramId > 0) {
            ...
            $.ajax({
                ...
                url: '/AllQuestions/GetSubProgram',
                success: function (result) {
                    for (var i = 0; i < result.length; i++) {
    ...
    $('#SubProgramBox').on('change', function () {
        ...
        if (BcscMapperViewModel.SubprogramId > 0) {
            ...
            $.ajax({
                ...
                url: '/AllQuestions/GetSubSpecFromSpecialityId',
                success: function (result) {
                    for (var i = 0; i < result.length; i++) {
            ...
            $.ajax({
                ...
                url: '/AllQuestions/GetSubSpeciality',
                success: function (result) {
                    for (var i = 0; i < result.length; i++) {
    ...
    $('#SubSpecialityBox').on('change', function () {
        ...
        if (BcscMapperViewModel.SubSpecialityId > 0) {
            ...
            $.ajax({
                ...
                url: '/AllQuestions/GetTopicByTopicId',
                success: function (result) {
                    Topic = '<option value="-1" disabled selected hidden>' + "Select Chapter, Topic" + '</option>';
                    for (var i = 0; i < result.length; i++) {
            ...
            $.ajax({
                ...
                type: 'GET',
                data: { subSpecialityId: SubSpecialityId },
                url: '/AllQuestions/GetTopicBySubSpec',
                success: function (result) {
                    Topic = '<option value="-1" disabled selected hidden>' + "Select Chapter, Topic" + '</option>';
                    for (var i = 0; i < result.length; i++) {
    ...
    function SearchQuestions(pageIndex) {
        ...
        if (TopicId == "Select Chapter, Topic" || TopicId == "0" || TopicId == "-1") {
            TopicId = null;

            window.location.href = '/AllQuestions/GetQuesByTopic' + '?topicId=' + TopicId + '&subSpecialtyId=' + SubSpecialtyId + '&programId=' + ProgramId + '&subProgramId=' + SubProgramId + '&pageIndex=' + pageIndex + '&noOfRecords=' + noOfRecords;
        }
        else {
            window.location.href = '/AllQuestions/GetQuesByTopic' + '?topicId=' + TopicId + '&subSpecialtyId=' + SubSpecialtyId + '&programId=' + ProgramId + '&subProgramId=' + SubProgramId + '&pageIndex=' + pageIndex + '&noOfRecords=' + noOfRecords;
        }
    ...
    $('.element').click(function () {
        ...
        if ($("#" + id + "ContainerDiv").html() == "") {
            $.ajax({
                type: 'GET',
                data: { questionId: id },
                /*url: '/AllQuestions/GetQuestionContent',*/
                url: '/AllQuestions/GetQuestionContent',
                    ...
                    if (result != '') {
                        ...
                        $.each(result, function (key, value) {
                            ...
                            $.ajax({
                                ...
                                /*url: '/AllQuestions/GetButtonStatus',*/
                                url: '/AllQuestions/GetButtonStatus',
            ...
            if (stringSpeciality.indexOf(idVal) == '-1') {
                ...
                if (!($("#" + id + "collapseInnerOne").hasClass("collapse in"))) {
                    ...
                    $.ajax({
                        ...
                        /*url: '/AllQuestions/GetUpdateContent',*/
                        url: '/AllQuestions/GetUpdateContent',
        ...
        $("#" + questionId + "SaveText").click(function () {
            ...
            if (textAreaData == '') {
                ...
                $.ajax({
                    ...
                    url: '/AllQuestions/InsertContent',
                    contentType: "application/x-www-form-urlencoded",
                    dataType: "json",
                    success: function (result) {
                        if (result == 1) {
    ...
    function getBCSCHTMLData(subSpecialityId, ChapterName, questionId) {
        ...
        $.ajax({
            type: 'GET',
            data: { subSpecialityId: subSpecialityId },
            /*url: '/AllQuestions/GetBCSCSectionNumber',*/
            url: '/AllQuestions/GetBCSCSectionNumber',
            crossDomain: true,
            success: function (result) {
    ...
    function displayQuestDetails(questionId, subSpecialityId) {
        $.ajax({
            ...
            /*url: '/AllQuestions/GetBCSCQuestDetails',*/
            url: '/AllQuestions/GetBCSCQuestDetails',
            success: function (result) {
