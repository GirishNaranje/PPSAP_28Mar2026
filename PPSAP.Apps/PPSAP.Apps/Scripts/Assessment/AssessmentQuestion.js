...
$(document).ready(function () {
    ...
    */
    ...
    function BookmarkQuestion() {
        var val = ($("#markQuestion").attr("alreadyMarked") === "true");
        $.ajax({
    ...
    function hotKeyPressed() {
        ...
        if (event.type === 'keydown' && [65, 66, 67, 68, 78, 110, 80, 112, 77, 109].indexOf(event.keyCode) !== -1) {

            //For BookMark
            if (!($("#Markstatus").is(':disabled')) || !($("#markQuestion").is(':disabled'))) {
                if (event.keyCode == 77 || event.keyCode == 109) {
                    if ($('#Markstatus').length > 0 || $('#markQuestion').length > 0) {
                        BookmarkQuestion();
                    }
                    else {
                        return;
