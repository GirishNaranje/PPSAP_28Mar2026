    ...
    public static class AssessmentBL
    {
        ...
        public static QuestionDetails GetExamQuestion(int? examId, int? questionId, int? userId)
        {
            ...
            try
            {
                ...
                // Get Question Choice by Question Id
                choiceList = AssessmentDAL.GetChoiceBYQuestionId(Convert.ToInt32(questionId), examId, questionDetails.examAttemptAnswerDTO.ChoiceId, questionDetails.examAttemptId, connection);
                questionDetails.choiceList = choiceList;
        ...
        public static QuestionDetails GetExamQuestion_Search(int? examId, int? questionId, int? userId)
        {
            try
            {
                ...
                if (examId == 0 && examId != null)
                {
                    choiceList = AssessmentDAL.GetChoiceBYQuestionId(Convert.ToInt32(questionId), examId, questionDetails.examAttemptAnswerDTO.ChoiceId, questionDetails.examAttemptAnswerDTO.ExamAttemptId);
                    questionDetails.choiceList = choiceList;
                }
                else
                {
                    choiceList = AssessmentDAL.GetChoiceBYQuestionId(Convert.ToInt32(questionId), examId, questionDetails.examAttemptAnswerDTO.ChoiceId, questionDetails.examAttemptId);
                    questionDetails.choiceList = choiceList;
                }
